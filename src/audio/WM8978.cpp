...
uint8_t WM8978::Write_Reg(uint8_t reg, uint16_t val)
{
  ...
  buf[1] = val & 0XFF;
  Wire1.beginTransmission(WM8978_ADDR); // 发送数据到设备号为4的从机
  Wire1.write((const uint8_t *)buf, 2);
  Wire1.endTransmission(); // 停止发送
...
void WM8978::cfgInput(uint8_t micen, uint8_t lineinen, uint8_t auxen)
{
  ...
  if (lineinen)
    WM8978::setLINEINgain(5); // LINE IN 0dB增益
  else
    WM8978::setLINEINgain(0); // 关闭LINE IN
  if (auxen)
    WM8978::setAUXgain(7); // AUX 6dB增益
  else
    WM8978::setAUXgain(0); // 关闭AUX输入
}
// WM8978 输出配置
...
bool WM8978::begin()
{
  Wire1.beginTransmission(WM8978_ADDR);
  const uint8_t error = Wire1.endTransmission();
  if (error)
  {
    log_e("No WM8978 dac @ i2c address: 0x%X, error:%d", WM8978_ADDR, error);
    return false;
  }
  const int err = Init();
  if (err)
...
{
  if (!Wire1.begin(sda, scl, frequency))
  {
