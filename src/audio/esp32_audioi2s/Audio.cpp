...
Audio::Audio(bool internalDAC /* = false */, uint8_t channelEnabled /* = I2S_DAC_CHANNEL_BOTH_EN */, uint8_t i2sPort)
{
    ...
    {
        // m_i2s_config.mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_TX | I2S_MODE_RX); // 增加I2S_MODE_RX，录音功能
        m_i2s_config.mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX | I2S_MODE_TX); // 增加I2S_MODE_RX，录音功能
...
void Audio::processRecord() // 录音
{

    if (!(audiofile && m_f_recordering))
        return;
...
void Audio::StopRecord() // 停止录音
{
    ...
    m_i2s_config.mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_TX); // 恢复 TX模式
    i2s_driver_install((i2s_port_t)m_i2s_num, &m_i2s_config, 0, NULL);
    I2Sstart(m_i2s_num);
    */
...
esp_err_t Audio::RecordToSD(fs::FS &fs, const char *path)
{
    ...
    AUDIO_INFO(sprintf(chbuf, "record to file: \"%s\"", audioName); vTaskDelay(2);)

    audiofile = fs.open(audioName, "w", true);
    ...
    if (!audiofile)
    {
        ...
        {
            vTaskDelay(2);
            audio_info("Failed to open file for recording");
        }
        return false;
    }
    AUDIO_INFO(sprintf(chbuf, "open file success: "); vTaskDelay(2);)
...
void Audio::httpPrint(const char *url)
{
    ...
    if (!_client->connected())
    {
        ...
        if (m_f_m3u8data && m_playlistBuff)
            strcpy(m_playlistBuff, url); // save new m3u8 chunklist
    }
    _client->print(resp);
...
bool Audio::connecttohost(const char *host, const char *user, const char *pwd)
{
    ...
    {
        if (audio_info)
            audio_info("Hostaddress is wrong");
        if (l_host)
        {
            free(l_host);
            l_host = NULL;
    ...
    if (p < 0)
    { // http not found, shift right +7, then insert http://
...
bool Audio::latinToUTF8(char *buff, size_t bufflen)
{
    ...
    {
        len = strlen(buff);
        if (buff[pos] >= 0x80 && buff[pos + 1] < 0x80)
        { // is not UTF8, is latin?
...
int Audio::read_WAV_Header(uint8_t *data, size_t len)
{
    ...
    if (m_controlCounter == 8)
    {
        ...
        { // sometimes there is nothing here
            ...
            if (m_f_webfile)
                m_audioDataSize = m_contentlength - headerSize;
        }
...
int Audio::read_OGG_Header(uint8_t *data, size_t len)
{
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    if (m_controlCounter == OGG_HEADER)
    ...
    { /* check OGG FIRST PAGES (has no streaming content) */
        uint8_t i = 0;
...
void Audio::processPlayListData()
{
    ...
    while (true)
    {
        ...
        if (pos == 254)
        {
            pl[pos] = '\0'; /*log_e("playlistline overflow");*/
        }
    }
    ...
    { // Read header
        ...
        if (m_f_Log)
        {
            AUDIO_INFO(sprintf(chbuf, "Playlistheader: %s", pl);)
        } // Show playlistheader
        ...
        if ((indexOf(pl, "Connection:close", 0) >= 0) && !f_ct)
        { // #193 is not a playlist if no ct found
        ...
        if (pos >= 0)
        {
            m_datamode = AUDIO_NONE;
            if (audio_info)
                audio_info("Error 400 Bad Request");
            stopSong();
            return;
        }
        ...
        if (pos >= 0)
        {
            m_datamode = AUDIO_NONE;
            if (audio_info)
                audio_info("Error 404 Not Available");
            stopSong();
            return;
        }
        ...
        if (pos >= 0)
        {
            m_datamode = AUDIO_NONE;
            if (audio_info)
                audio_info("Error 401 Unauthorized");
            stopSong();
            return;
        }
        ...
        if (pos >= 0)
        {
            m_datamode = AUDIO_NONE;
            if (audio_info)
                audio_info("Error 403 Forbidden");
            stopSong();
            return;
        }
        ...
        if (startsWith(pl, "icy-"))
        { // icy-data in playlist? that can not be
            m_datamode = AUDIO_HEADER;
            if (audio_info)
                audio_info("playlist is not valid, switch to AUDIO_HEADER");
            return;
        }
        ...
        if (startsWith(pl, "location:") || startsWith(pl, "Location:"))
        {
            ...
            pos = indexOf(pl, "/", 10);
            if (strncmp(host, m_lastHost, pos) == 0)
            { // same host?
                _client->stop();
                _client->flush();
                httpPrint(host);
                ...
                connecttohost(host); // different host,
        }
        return;
    ...
    { // Read next byte of .m3u file data
        if (m_f_Log)
        {
            AUDIO_INFO(sprintf(chbuf, "Playlistdata: %s", pl);)
        } // Show playlistdata
        ...
        if (m_playlistFormat == FORMAT_M3U)
        {
            ...
            if (!f_begin)
                f_begin = true; // first playlistdata received
            if (indexOf(pl, "#EXTINF:", 0) >= 0)
            {                              // Info?
        ...
        if (m_playlistFormat == FORMAT_PLS)
        {
            ...
            if (!f_begin)
            {
                ...
                else
                {
                    ...
                    if (audio_info)
                        audio_info("pls is not valid, switch to AUDIO_HEADER");
                    return;
                }
            }
            ...
            if (startsWith(pl, "Length1"))
                f_title = true; // if no Title is available
            if ((f_ref == true) && (strlen(pl) == 0))
                f_title = true;

            if (indexOf(pl, "Invalid username", 0) >= 0)
            { // Unable to access account: Invalid username or password
                m_f_running = false;
                stopSong();
                m_datamode = AUDIO_NONE;
                return;
        ...
        { // Advanced Stream Redirector
            ...
            if (indexOf(pl, "http", 0) == 0 && !f_entry)
            {                                       // url only in asx
        ...
        if (m_playlistFormat == FORMAT_M3U8)
        {
            ...
            if (!f_begin)
            {
                ...
                else
                {
                    ...
                    m_playlistFormat = FORMAT_NONE;
                    if (audio_info)
                        audio_info("m3u8 is not valid, switch to AUDIO_HEADER");
                    return;
                }
            }
            ...
            if (startsWith(pl, "#EXT-X-TARGETDURATION:"))
            {
                targetDuration = atoi(pl + 22);
            }
            ...
            if (f_ExtInf)
            {
                ...
                if (plsEntry == 0)
                {
                    ...
                    if (m_m3u8codec == CODEC_M4A)
                    {
                        ...
                        for (int i = p1; i < p2; i++)
                        {
                            if (entryPos[i] >= 48 && entryPos[i] <= 57)
                            { // numbers only
                                if (!seqNrPos)
                                    seqNrPos = i;
                            }
            ...
            {
                if (plsEntry > 0)
                { // we have found some (url) entries
...
void Audio::processM3U8entries(uint8_t _nrOfEntries, uint32_t _seqNr, uint8_t _seqNrpos, uint16_t _targetDuration)
{
    ...
    if (_nrOfEntries == 0)
    { // processM3U8entries(0,0,0,0)
        if (currentSeqNr < maxSeqNr)
        { // next entry in playlist
            char sBuff[12];
            itoa(currentSeqNr, sBuff, 10);
    ...
    if (_nrOfEntries > 0)
    { // e.g. processM3U8entries(3,123456,55,10)

        while (currentSeqNr > _seqNr)
        {
            _seqNr++;
            _nrOfEntries--;
        };
...
void Audio::processWebStream()
{
    ...
    if (m_f_webfile && byteCounter == m_contentlength)
    {
        ...
        {
            if (m_datamode == AUDIO_DATA && m_f_m3u8data)
            {
                processM3U8entries();
                return;
            } // we need the next audioChunk
        }
        if (InBuff.bufferFilled() > 0)
        {
            if (InBuff.bufferFilled() == 128)
            { // post tag? comes sometimes after podcasts
    ...
    if (ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_DEBUG)
    {
        ...
        {
            if (t0 && (millis() - t0) > 400)
                log_d("\033[31m%dms no data received", millis() - t0);
            t0 = 0;
        }
        i++;
        if (i == 10)
    ...
    if (m_f_chunked && !m_chunkcount && availableBytes)
    ...
    { // statement has no effect
        ...
        if (!m_f_swm)
            bytesCanBeWritten = min(metacount, bytesCanBeWritten);
        if (m_f_chunked)
            bytesCanBeWritten = min(m_chunkcount, bytesCanBeWritten);

        int16_t bytesAddedToBuffer = 0;

        if (InBuff.havePSRAM())
            if (bytesCanBeWritten > 4096)
                bytesCanBeWritten = 4096; // PSRAM throttle
        ...
        if (m_f_webfile)
        {
            ...
            if (byteCounter + bytesCanBeWritten >= m_contentlength)
                bytesCanBeWritten = m_contentlength - byteCounter;
        }
    ...
    if (m_f_webfile && m_controlCounter != 100 && !m_f_m3u8data)
    { // m3u8call, audiochunk has no header
...
void Audio::processAudioHeaderData()
{
    ...
    {                            // audio header complete?
        ...
        if (m_f_Log)
        {
            AUDIO_INFO(sprintf(chbuf, "Switch to DATA, metaint is %d", m_metaint);)
        }
    ...
    if (indexOf(hl, "ICY 401", 0) >= 0)
    {
        ...
        stopSong();
        if (audio_info)
            audio_info("ICY 401 Service Unavailable");
        return;
    }

    if (indexOf(hl, "content-type:", 0) >= 0)
    {
        if (parseContentType(hl))
            m_f_ctseen = true;
    }
    ...
    else if (startsWith(hl, "icy-name:"))
    {
        ...
        c_icyname += idx; // Remove leading spaces
        idx = strlen(c_icyname);
        while (c_icyname[idx] == ' ')
        {
            idx--;
        }
        c_icyname[idx + 1] = 0; // Remove trailing spaces
    ...
    else if (startsWith(hl, "content-length:"))
    {
        ...
        if (m_f_Log)
        {
            AUDIO_INFO(sprintf(chbuf, "content-length: %i", m_contentlength);)
        }
    }
    else if (startsWith(hl, "icy-description:"))
    {
        ...
        if (audio_icydescription)
            audio_icydescription(c_idesc);
        f_icydescription = true;
    }
    else if ((startsWith(hl, "transfer-encoding:")))
    {
        if (endsWith(hl, "chunked") || endsWith(hl, "Chunked"))
    ...
    else if (startsWith(hl, "icy-url:"))
    {
        ...
        icyurl += idx; // remove leading blanks
    ...
    {
        if (isascii(hl[0]) && hl[0] >= 0x20)
        { // all other
            if (m_f_Log)
            {
                AUDIO_INFO(sprintf(chbuf, "%s", hl);)
            }
        }
    }
    return;
...
bool Audio::readMetadata(uint8_t b, bool first)
{
    ...
    {                         // First byte of metadata?
        ...
        pos_ml = 0;
        chbuf[pos_ml] = 0; // Prepare for new line
    }
    else
    {
        ...
        if (pos_ml == 509)
        {
            log_e("metaline overflow in AUDIO_METADATA! metaline=%s", chbuf);
        }
        if (pos_ml == 510)
        {
...
void Audio::showstreamtitle(const char *ml)
{
    ...
    { // Streamtitle found
        ...
        if (m_streamTitleHash != hash)
        {
            ...
            if (audio_showstreamtitle)
                audio_showstreamtitle(sTit + pos);
        }
        free(sTit);
    }
...
int Audio::sendBytes(uint8_t *data, size_t len)
{
    ...
    if (m_codec == CODEC_OGG_FLAC)
        ret = FLACDecode(data, &bytesLeft, m_outBuff); // FLAC webstream wrapped in OGG
...
bool Audio::setFilePos(uint32_t pos)
{
    ...
    if (m_avr_bitrate)
        m_audioCurrentTime = ((pos - m_audioDataStart) / m_avr_bitrate) * 8; // #96
